...
class ProcessGAN_Time:
    ...
    def train(self, target, aut_data, aut_data_time, aut_data_duration):
        ...
        test_data, _, _ = torch.utils.data.random_split(dataset_2, (self.train_size, self.valid_size, self.test_size),
                                                        generator=torch.Generator().manual_seed(self.seed))
        test_dataloader = DataLoader(test_data, batch_size=self.train_size, drop_last=False, shuffle=False,
                                     num_workers=1)
        ...
        pre_epoch = self.epochs_pre_train  # 50 # This setting overwrites the setting in the configuration file (200)
        if self.model_mode != 'vanilla':
            mean_act_loss, mean_gen_loss, mean_time_loss, mean_act_time_loss = self.get_pre_exp_loss(pre_epoch, g_model,
                                                                                                     d_model,
                                                                                                     train_dataloader)
        ...
        for big_epoch in range(1, self.epochs + 1):
            ...
            for i, item in enumerate(train_dataloader):
                ...
                gen_loss, g_output_act, g_output_time = self.generator(random_data, g_model, d_model, batch,
                                                                       d_criterion, real_labels, dis_data_duration,
                                                                       dis_data_time)
                ...
                if big_epoch % self.gd_ratio == 0:
                    ...
                    dis_loss, gd_acc_neg, gd_acc_pos = self.discrminator(dis_data_pos, dis_data_time, g_output_act,
                                                                         g_output_time, g_model, d_model, d_criterion,
                                                                         batch)
            ...
            if big_epoch % self.gd_ratio == 0:
                ...
                print('epoch {:3d} |  '
                      ...
                      'per_time {:5.4f}s | '
                      'remain_time {}'
                      .format(big_epoch, gen_total_loss, dis_total_loss, gd_acc_pos, gd_acc_neg, acc, per_epoch_time,
                              es_remain_time))
            ...
            if big_epoch % 100 == 0:
                ...
                with torch.no_grad():
                    gen_list, gen_list_time = gen_data_from_rand(self.gen_num, g_model, self.vocab_num_act, self.device,
                                                                 str(big_epoch) + '_result_trans', self.save_path,
                                                                 self.seq_len, aut_data_duration)
                # Evaluate and record the results.
                with open(self.save_path + 'stats/' + 'dif_log.txt', 'a') as filehandle:
                    filehandle.write('%s\n' % big_epoch)
    ...
    def generator(self, data, g_model, d_model, batch, d_criterion, real_labels, duration, target_time):
        ...
        pad_mask_mul, pad_mask_add = get_pad_mask(g_output_act, batch, self.seq_len, self.vocab_num_act + 1,
                                                  self.pad_ind_act, self.device)
    ...
    def discrminator(self, dis_data_pos, dis_data_time, g_output_act, g_output_time, g_model, d_model, d_criterion,
                     batch):
    ...
    def get_pre_exp_loss(self, pre_epoch, g_model, d_model, train_dataloader):
        ...
        for epoch in range(pre_epoch):
            ...
            for i, item in enumerate(train_dataloader):
                ...
                gen_loss, pre_g_output_t, pre_g_output_time = self.generator(data, g_model, d_model, batch, d_criterion,
                                                                             real_labels, dis_data_duration,
                                                                             dis_data_time)
                ...
                act_time_loss = self.get_act_time_loss(dis_data_pos, dis_data_time, pre_g_output_t, pre_g_output_time,
                                                       batch)
